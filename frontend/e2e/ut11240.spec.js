...
test('instructor reviews five variants and confirms the shared batch', async ({ page }) => {
  ...
  await page.getByRole('button', { name: /Пожар в образовательном учреждении/ }).click()
  await page.getByLabel('Занятие').selectOption('4')
  await page.getByRole('button', { name: 'Сформировать 5 карточек' }).click()
  ...
  await page.getByLabel('Распределение').selectOption('group:6')
  await page.getByRole('button', { name: 'Утвердить набор и добавить в занятие' }).click()
  await expect(page.getByText('Набор утверждён и добавлен в занятие.')).toBeVisible()
