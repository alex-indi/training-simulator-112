...
**Статус:** legacy/internal описание расширенного Scenario Engine. Его таблицы и выпуск ранее подготовленных событий остаются в backend для совместимости, но конструктор событий, ручные сообщения служб и `expected_actions` не входят в пользовательский MVP workflow. Действующая модель — [текущее состояние](current-state.md) и [библиотека карточек MVP](scenario-library-and-generator.md).
**Примечание:** дальнейшие разделы фиксируют историческую расширенную модель и не являются инструкцией по созданию обычного шаблона преподавателем.
