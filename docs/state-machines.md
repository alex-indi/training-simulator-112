...
Документ фиксирует текущие автоматы карточки и целевую модель учебной смены по [концепции кабинета преподавателя](instructor-workspace-concept.md). Состояния пауз и завершения занятия описаны как целевые; базовый код UT112-6—9 ещё реализует только `DRAFT → READY → ACTIVE` для `TrainingSession`.

Главная цель — не смешивать несколько разных состояний в одно поле `status`.

В системе существуют как минимум пять независимых состояний/автоматов:
...
Новые карточки не выдаются `TrainingRun.PAUSED`; его уже взятые карточки сохраняются. При `TrainingSession.FINISHING` активный участник может завершать взятые карточки без новой выдачи. Закрытие всех незавершённых прохождений переводит сессию в `COMPLETED`; при `IMMEDIATE` незавершённые прохождения получают `ABORTED`.
