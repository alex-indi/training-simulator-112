...
Документ фиксирует текущие автоматы карточки и целевую модель учебной смены по [концепции кабинета преподавателя](instructor-workspace-concept.md). Состояния пауз и завершения занятия описаны как целевые; текущий код реализует `DRAFT → READY → ACTIVE` для `TrainingSession`, но ещё не реализует глобальную и личную паузы UT112-21.
