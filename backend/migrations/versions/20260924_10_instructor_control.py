...
def upgrade() -> None:
    ...
    op.add_column(
        "training_runs", sa.Column("paused_seconds", sa.Float(), nullable=False, server_default="0")
    )
    op.create_table(
        "instructor_actions",
    ...
    op.create_index(
        "ix_instructor_actions_training_session_id", "instructor_actions", ["training_session_id"]
    )
    op.create_table(
        "session_pauses",
        sa.Column("id", sa.Integer(), primary_key=True),
    ...
    op.create_index(
        "ix_session_pauses_training_session_id", "session_pauses", ["training_session_id"]
    )
    op.create_table(
        "run_pauses",
        sa.Column("id", sa.Integer(), primary_key=True),
...
def downgrade() -> None:
    for table, index in (
        ("scenario_events", "ix_scenario_events_incident_id"),
        ("instructor_notes", "ix_instructor_notes_training_run_id"),
        ("run_pauses", "ix_run_pauses_training_run_id"),
        ("session_pauses", "ix_session_pauses_training_session_id"),
        ("instructor_actions", "ix_instructor_actions_training_session_id"),
    ):
